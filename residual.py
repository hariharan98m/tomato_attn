...
def ResidualConvNet(input_shape):
    ...
    residual_input_2= Add()([block_2_in1_max, X])
    
    # Second Block
    #CONV layer
    conv_2_out = Conv2D(64, (5, 5), strides = (2,2), activation='relu', padding='same')(residual_input_2)
    # MAXPOOL
    ...
    residual_input_3= Add()([block_3_in2_max, block_3_in1_max, X])
    
    # Third Block
    conv_3_out = Conv2D(128, (5,5), strides = (1, 1),activation='relu', padding='same')(residual_input_3)
    # MAXPOOL
...
with tf.device('/device:GPU:0'):
    ...
    new_model = ResidualConvNet((256, 256, 3))

    ## Compling the model
    new_model.compile(optimizer = "adam" , loss = loss, metrics = ["accuracy"]);

    new_model.load_weights("saveResidualWeights/residualWeights.hdf5")
    
    ## Printing the modle summary
    print(new_model.summary())
    
    # checkpoint
    filepath="saveResidualWeights/residualWeights.hdf5"
...
#        print ("Evaluating on fold" + str(index+1) +"/5")
       ...
       # print(model.evaluate_generator(val_seq)
        ...
        # list all data in history
            ...
            plt.show()
            plt.savefig('saveResidualWeights/acc'+str(index+1)+ '.png')
            ...
            plt.legend(['train', 'val'], loc='upper left')
            plt.show()
            plt.savefig('saveResidualWeights/loss'+str(index+1)+ '.png')
        except: 
            #save model
            print('error occurred')
            new_model.save('saveResidualWeights/saveModel_fold'+str(index+1)+'.h5')            
