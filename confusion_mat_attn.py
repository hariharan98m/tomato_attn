...
def plot_confusion_matrix(y_true, y_pred, classes,
    ...
    classes = ['Late Blight', 'Early Blight', 'Leaf Mold', 'Healthy']
    if normalize:
...
with tf.device('/device:GPU:0'):
    ...
    new_model = load_model('saveAttentionWeights/saveModel_fold5.h5', custom_objects={ 'loss': loss })
    ## Compling the model
...
#        print ("Evaluating on fold" + str(index+1) +"/5")
        ...
        val_seq = InfectedLeavesSequence(X_val, y2_val, batch_size = 1)
        
        predict = new_model.predict_generator(val_seq)
        print('y true shape', y2_val.shape)
        print('predict shape', predict.shape)
        
        predicted = np.argmax(predict, axis=-1)
        ground_truth = np.argmax(y2_val, axis=-1)
        
        plot_confusion_matrix(ground_truth, predicted, classes=None,
                          normalize=False,
                          title='Confusion Matrix for Attention Model')
        
        plt.savefig('Confusion matrix attn.png')
        
        print('predict seq: ', predict)
        print('predict seq shape: ', predict.shape)
